...
// Helper function to convert u256 vector to u8 vector (big endian)
fun u256_to_u8_bytes(s2_u256: &vector<u256>): vector<u8> {
...
// CLI-friendly version - original that accepts u8 signature
public fun verify_signature_cli(
    ...
    assert!(result, 1); // assert on failure
    result // return result
}

// CLI-friendly version that accepts u256 signature (s2 part)
public fun verify_signature_cli_u256(
    public_key_compacted: vector<u256>,
    message: vector<u8>,
    salt: vector<u8>,
    signature_s2_u256: vector<u256>,
): bool {
    // Convert s2 from u256 to u8 bytes
    let s2_bytes = u256_to_u8_bytes(&signature_s2_u256);

    // Reconstruct full signature: salt + s2_bytes
    let mut full_signature = salt;
    vector::append(&mut full_signature, s2_bytes);

    let result = verify_signature(&public_key_compacted, &message, &full_signature);
    assert!(result, 1); // assert on failure
    result // return result
}
...
#[test]
fun test_browser_vector() {
    std::debug::print(&std::string::utf8(b"=== browser Test Vector ==="));

    // Message: "We are SuiQ!"
    let message = b"We are SuiQ!";

    // Create signature in ETHFALCON format: salt (40 bytes) + s2 compacted (32 * 32 bytes)
    let mut signature = vector::empty<u8>();

    // Salt from test vector 0 (40 bytes)
    let salt = x"db1568797d4d37b6f222c8522245e86a82ed3fea90c66e3432c6dff93a6d4792e95d5b5d30298939";
    vector::append(&mut signature, salt);

    // S2 signature in compacted u256 format (32 values)
    let s2_u256 = vector[21315244445401374779589209277068828934886174346974694207624460329193909780623u256, 21133268773382942888954316721125136081167117632436948750939388805881526234991u256, 92204446480474887635710633947173493236570765739583268608526256784095587982u256, 21620908129804789478235245743790902281373459971535914748056888110776265617296u256, 21329703711792024701878605256710067751055343792393895717403808113576346660641u256, 21608542950305056362167111429751106329093709108757649500662713359224163741552u256, 21467520290227560151894176116352334056261346916434992347192283738092449378282u256, 17993396945115908932166691085338183521835923431061177714804205111275630428u256, 21454831677087350123984904950521078396361794948388587995213152202622888640556u256, 21698654868644055958692983412345021639770508029824758996736587107864093732684u256, 21515227587296830481058691051667601167613130169253381517515565498494742823044u256, 189057547696570915149971581126850177275378076949965174450760416841721708609u256, 21396848159975776438410961888372868090163788940756323301840736578351708254160u256, 21580272340848917248315975986330850671265536091492768390278032077133455896396u256, 21329381033144157904501108935289126768826147887519210410428860792194161508517u256, 21668941259890475893067772343658540489721126510717064927229984245115569569974u256, 420839515277981439668956145139055189998561313798404904380435671797598793714u256, 137814286743394923779558367197435901432546583550068059647609690213642612681u256, 339236151179108159544440250893847496585374099508079445280515382770242945116u256, 183752800600816066175771628687618015925228023411675022585181354541504725184u256, 137817090569284652157818816407985967915953568779334971564718401763376574357u256, 210256229598218326189623664356836150189213605198499836401680803956800291133u256, 21547029729105925746684796072773570710096307648750078931601610026555907702827u256, 28271094763152895780739075094657542