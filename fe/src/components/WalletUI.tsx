...
export const WalletUI: React.FC<WalletUIProps> = ({ onDeposit }) => {
  ...
  useEffect(() => {
    if (hybridWallet && hybridWalletId) {
      ...
      console.log(
        ...
        Number(hybridWallet.treasury) / Number(MIST_PER_SUI)
      );
      console.log("Transaction Nonce:", hybridWallet.nonce);
      console.log("Ed25519 Public Key:", hybridWallet.ed25519_pubkey);
      console.log(
        "Falcon Public Key Length:",
        hybridWallet.falcon_pubkey?.length || 0
      );
  ...
  const handleGenerateFalconKeys = async () => {
    if (!isFalconReady) {
      alert(
        "Falcon is not ready yet. Please wait for initialization to complete."
      );
      return;
    }
    ...
    try {
      ...
      alert(
        "🎉 Falcon keys generated successfully! Keys are now stored securely."
      );
    } catch (error) {
  ...
  const handleGenerateWalletWithFalcon = async () => {
    ...
    try {
      ...
      try {
        ...
        tx.moveCall({
          target: `${
            process.env.REACT_APP_PACKAGE_ID || "0x1"
          }::falcon_wallet::register_falcon_key`,
          arguments: [
        ...
        const result = await new Promise((resolve, reject) => {
          signAndExecuteTransaction(
            ...
            },
            {
              onSuccess: (result) => {
                console.log(
                  "✅ Move contract wallet created successfully!",
                  result.digest
                );
                resolve(result);
              },
        ...
        console.warn(
          "⚠️ Move contract call failed, proceeding with local storage only:",
          contractError
        );
  ...
  const [needsFalconKeyGeneration, setNeedsFalconKeyGeneration] =
    useState(false);
  ...
  useEffect(() => {
    if (currentAccount && isFalconReady) {
      ...
      if (!wallet.hasFalconKeys()) {
        ...
        console.log(
          "⚠️ WalletUI: No Falcon keys found - showing generation UI"
        );
      } else {
      ...
      >
        ...
        >
          <div style={{ marginBottom: "1rem" }}>
            ...
            <br />
            🏗️ Creating Move contract wallet
            <br />
